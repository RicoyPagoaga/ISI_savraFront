...
const cargos = () => {
    ...
    const saveProduct = () => {
        ...
        if (product.name.trim()) {
            ...
            if (product.id) {
                ...
                toast.current.show({ severity: 'success', summary: 'Éxitoso', detail: 'Cargo Actualizado', life: 3000 });
            } else {
                _product.id = createId();
                _product.image = 'product-placeholder.svg';
                _products.push(_product);
                toast.current.show({ severity: 'success', summary: 'Éxitoso', detail: 'Cargo Registrado', life: 3000 });
            }
    ...
    const deleteProduct = () => {
        ...
        setDeleteProductDialog(false);
        setProduct(emptyProduct);
        toast.current.show({ severity: 'success', summary: 'Éxitoso', detail: 'Cargo Eliminado', life: 3000 });
    };
            ...
            <div className="col-12">
                <div className="card">
                    ...
                    >
                        ...
                        <Column field="Nombre" header="Descripcion" sortable body={nameBodyTemplate} headerStyle={{ minWidth: '15rem' }}></Column>
                    ...
                    <Dialog visible={productDialog} style={{ width: '450px' }} header="Registro Países" modal className="p-fluid" footer={productDialogFooter} onHide={hideDialog}>
                        ...
                        <div className="field">
                            <label htmlFor="name">Descripcion del Cargo</label>
                            <InputText id="name" value={product.name} onChange={(e) => onInputChange(e, 'name')} required autoFocus className={classNames({ 'p-invalid': submitted && !product.name })} />
                            {submitted && !product.name && <small className="p-invalid">Name is required.</small>}
                        </div>
                    </Dialog>
                    ...
                    <Dialog visible={deleteProductsDialog} style={{ width: '450px' }} header="Confirmación" modal footer={deleteProductsDialogFooter} onHide={hideDeleteProductsDialog}>
                        <div className="flex align-items-center justify-content-center">
                            <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                            {product && <span>Esta seguro que desea eliminar este Cargo?</span>}
                        </div>
                    </Dialog>
                </div>
            </div>
